...
function options(context = contextWith()) {
  return {
    analysisId: "analysis:nut-017.3:bridge-001",
...
await test("Köprü cihaz-içi, sağlayıcısız ve Connect'siz durum bildirir", () => {
  ...
  assert.equal(status.engineVersion, "0.3.0-analysis");
...
await test("Eşleşmeyen kayıt için çıktı uydurulmaz", () => {
  ...
  assert.deepEqual(clone(result), {
    success: false,
    errorCode: "TODAY-AI-ANALYSIS-NO-MATCH",
    analysisError: "no-matching-rule"
  });
});
...
console.log(`NUT-017.3 Analysis Bridge: ${passed}/${results.length} başarılı`);
